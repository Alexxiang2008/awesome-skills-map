---
name: video-wrapper
description: 为访谈视频添加综艺特效（花字、卡片、人物条、章节标题等）。支持 YouTube/Bilibili 链接自动下载，支持 4 种视觉主题，先分析字幕内容生成建议供用户审批，再渲染视频。
argument-hint: <video-url-or-file> [subtitle-file] [config.json] [output.mp4]
user-invocable: true
...
### 方式一：视频链接（推荐）
直接提供 YouTube 或 Bilibili 视频链接，自动下载视频和字幕：
```
/video-wrapper https://www.youtube.com/watch?v=xxxxx
/video-wrapper https://www.bilibili.com/video/BVxxxxx
```
...
当用户提供视频链接时：
1. 检测视频平台（YouTube 或 Bilibili）
2. 使用 yt-dlp 自动下载视频
3. 自动下载字幕（优先中文，其次英文）
4. 如果没有字幕，提示用户使用语音识别工具

### 第一步：分析字幕内容

当有视频和字幕文件后，分析字幕内容，生成特效建议：
