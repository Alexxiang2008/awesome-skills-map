---
name: email-triage
description: "智能邮件分类与回复助手 - 收件箱零焦虑。邮件优先级分类（紧急/重要/可延后）、智能回复草稿生成、待办事项自动提取。支持 /email-triage 或 /邮件分类 或 /收件箱 命令启动。"
license: MIT
---
...
切换到 Skill 所在目录，测试邮箱连接：

```bash
cd ~/.claude/skills/email-triage && python3 -c "
...
读取 `~/.claude/skills/email-triage/prompts/classify.md` 中的 prompt 模板，对邮件进行分类。
...
读取 `~/.claude/skills/email-triage/prompts/extract_todos.md` 中的 prompt 模板。
...
读取 `~/.claude/skills/email-triage/prompts/reply.md` 中的 prompt 模板。
...
获取的邮件会自动保存到 `~/.claude/skills/email-triage/cache/latest_emails.json`，
包含以下信息：
- `fetched_at`: 获取时间
- `account`: 邮箱账号
- `total_count`: 邮件总数
